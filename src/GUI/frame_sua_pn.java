...
DecimalFormat dff = new DecimalFormat("#,###");
        for (int i = 0; i < ctpnDTO.size(); i++) {
            ...
            data[i][1] = dff.format(item.getSoluong());
            data[i][2] = item.getMasize();
            data[i][3] = dff.format(item.getGianhap());
...
public void setValueAt(Object value, int row, int column) {
    // Khởi tạo DecimalFormat để định dạng dấu phẩy cho số
    DecimalFormat df = new DecimalFormat("#,###");

    // Xử lý cho cột "Số lượng"
    if (column == 1) { // Cột số lượng
        try {
            // Loại bỏ dấu phẩy trước khi parse
            int soluong = Integer.parseInt(value.toString().replace(",", ""));
            double gianhap = Double.parseDouble(tableModel.getValueAt(row, 3).toString().replace(",", ""));
            
            // Tính toán "Thành tiền"
            double thanhtien = soluong * gianhap;
            
            // Định dạng và cập nhật lại các cột
            super.setValueAt(df.format(soluong), row, 1); // Cập nhật số lượng
            super.setValueAt(df.format(gianhap), row, 3); // Định dạng lại giá nhập (nếu cần)
            super.setValueAt(df.format(thanhtien), row, 4); // Cập nhật thành tiền
            ...
            JOptionPane.showMessageDialog(null, "Vui lòng nhập giá trị hợp lệ!", "Lỗi", JOptionPane.ERROR_MESSAGE);
        }
    } else {
        // Nếu không phải cột số lượng thì gọi phương thức cha để xử lý bình thường

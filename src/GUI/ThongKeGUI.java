...
public class ThongKeGUI extends JPanel {
    ...
    ArrayList<ThongKeDTO> dstk;
    ArrayList<DoanhThuDTO> dsdt;
...
//        lblHeader.setText("THỐNG KÊ DOANH THU");
        ...
        dsdt = dtBus.listDoanhThu(data_filters);
        
        //Bảng
        ThongKeBus thkBus = new ThongKeBus();
        dstk = thkBus.listDoanhThu(data_filters);
        for (ThongKeDTO row : dstk) {
        ...
        //Biểu đồ
        BarChart_DoanhThu chart = new BarChart_DoanhThu(dsdt, data_filters,"Thống kê doanh thu","Đồng");
        pnBieuDo.removeAll();
...
//        lblHeader.setText("SẢN PHẨM BÁN CHẠY");
        ...
        dstk = new ArrayList<>();
        ThongKeBus thkBus = new ThongKeBus();
        dstk = thkBus.listBanChay(data_filters);
        for (ThongKeDTO row : dstk) {
            //gọi lại hàm format cho cách hiện thị cho các số có giá trị quá lớn hoặc nhở
            String soLuong = FormatInt.format(row.getSoLuong());
            String thanhTien = FormatInt.format(row.getThanhTien());
...
//        PieChart_BanChay demo = new PieChart_BanChay("Sản phẩm bán chạy", ds);
        BarChart_DoanhThu demo = new BarChart_DoanhThu(dstk, data_filters, "Sản phẩm bán chạy", "cái");
        pnBieuDo.removeAll();
